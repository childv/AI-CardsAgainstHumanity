...
def humanChooseBest(playerWithBlack, choices):
    shuffle(choices)
...
def aiChooseBest(playerWithBlack, choices):
    shuffle(choices)
    humor_prediction = predict_batch(choices)
    print("The results are in! In no particular order: ")
    for i in range(len(choices)):
        print ("Option " + str(i+1) + ": " + choices[i][1] 
            + " -> " + str(humor_prediction[i]))
    print("")
    funniest_sentence = np.argmax(humor_prediction)
    print("The AI chose option " + str(funniest_sentence + 1) + ".")
    return choices[funniest_sentence][0]
...
def runRound(game, players, playerTurn):
    ...
    for player in playersWhoChoose:
        ...
        print("\n\n\n\n")
    blackAI = playerWithBlack.isAI()
    playerWhoWins = aiChooseBest(playerWithBlack, choices) if blackAI \
     else humanChooseBest(playerWithBlack, choices)
